...
# setting the name of model and exp folder
MODEL_NAME = "two" + str(name_ID)
FOLDER_NAME = "exp_two" + str(name_ID)
# The name for all exp root folder
MODEL_ROOT_FOLDER_NAME = "two_layer_random_models/"
EXP_ROOT_FOLDER_NAME = "two_exp/"
...
WEIGHTS_DIM_LIST = [IDIM,DIM1,SHRINK,10]
...
class CIFAR10Net(nn.Module):
    def __init__(self):
        ...
        self.output_DIM = [DIM1,SHRINK,10]
        self.fc1 = nn.Linear(IDIM, DIM1)
        self.fc2 = nn.Linear(DIM1, SHRINK)
        self.fc3 = nn.Linear(SHRINK, 10)
        # self.relu = nn.ReLU()

        self.prelu1 = nn.PReLU(num_parameters=DIM1)
        self.prelu2 = nn.PReLU(num_parameters=SHRINK)
        self.double()
...
w_last = pytorch_state['fc3.weight'].cpu().numpy()
w_last = w_last.T
WEIGHTS.append(w_last)

b_last = pytorch_state['fc3.bias'].cpu().numpy()
