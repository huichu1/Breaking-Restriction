...
# Function to solve the intersection space of dual space
...
# Function to generate random points on the n-2 dimensional subspace with certain step size
# step size increase to avoid crossing the critical plane
...
# transfer model parameter to a prefix for extraction
...
def is_consistent_help(points, prefix, layer=0, do_return_soln=False, allow_close=False,step_size = 3000):
    ...
    if do_return_soln:
        ...
        points_subset = []
        hits = np.zeros([IDIM, DIM1, SHRINK][layer])
...
def is_consistent_slope_help(points, prefix, layer=0, do_return_soln=False, allow_close=False,step_size = 3000):
    ...
    if do_return_soln:
        ...
        points_subset = []
        hits = np.zeros([2*IDIM, 2*DIM1, 2*SHRINK][layer + 1])
    ...
    if do_return_soln:
        ...
        if input_dim_this_layer > input_dim_form_layer and target_space.shape[1] == input_dim_this_layer - input_dim_form_layer + 1:
            ...
            if have_null_space == False:
                ...
                # print("Let's Check!",file=DEBUG_FILE)
                # for target_index in range(len(coeff)):
                #     check_row = np.matmul(coeff[target_index,:],full_rank_weight)
                #     print("Check row!",file=DEBUG_FILE)
                #     print(check_row,file=DEBUG_FILE)
                #     print(real_weight[target_index + WEIGHTS_DIM_LIST[layer-1],:],file=DEBUG_FILE)
        ...
        if try_solution.cost > 1e-5 or try_solution.cost < 1e-17 or True:
            ...
            for i in range(0,10):
                ...
                if part_try_solution.cost < 1e-10:
                    ...
                    break
                # print(part_try_solution.x,file=RESULT_FILE)
