...
import time
import deep_layer_utils
import math

rebuttal_joint_FILE = open(EXP_ROOT_FOLDER_NAME  + FOLDER_NAME + "/joint_rebuttal_test.txt","a")
...
class CIFAR10NetPrefix(nn.Module):
    def __init__(self, layers):
        ...
        if layers == 0:
            ...
            self.layers = layers
            h = [DIM1,DIM2,DIM3,DIM4,DIM5,DIM6,DIM7,SHRINK,10]
...
def is_consistent_help(points, prefix, layer=0, do_return_soln=False, allow_close=False,step_size = 3000):
    ...
    if do_return_soln:
        ...
        if np.min(hits) == 0 and layer > 0:
            ...
            return None, None
        points_subset = []
        hits = np.zeros([IDIM, DIM1, DIM2, DIM3, DIM4, DIM5, DIM6, DIM7, SHRINK][layer])
        for coord in order:
            if hits[coord] >= 40:
                continue
...
def is_consistent_slope_help(points, prefix, layer=0, do_return_soln=False, allow_close=False,step_size = 3000):
    ...
    if vectorized_check_closest_pair_distance(points) and not allow_close:
        ...
        return None, None # rejected
    DIM_List = [IDIM,DIM1,DIM2,DIM3,DIM4,DIM5,DIM6,DIM7,SHRINK,10]
    ...
    if do_return_soln:
        ...
        if np.min(hits) == 0 and layer > 0:
            print("Hit some zero times. Mean OK", np.mean(hits!=0))
            print("Hit some zero times. Mean OK", np.mean(hits!=0),file=RESULT_FILE)
            print(list(hits))
            print("Reason 2",file=RESULT_FILE)
            return None, None
        points_subset = []
        hits = np.zeros([2*IDIM, 2*DIM1, 2*DIM2, 2*DIM3, 2*DIM4,2*DIM5, 2*DIM6,2*DIM7,2*SHRINK][layer + 1])
        for coord in order:
            if hits[coord] >= 8:
                continue
            for entry in np.where(hiddens[:, coord])[0][:2]:
    ...
    for cluster_id, maybe in sorted(cluster.items(), key=lambda x: len(x[1])):
        ...
        if True:
            ...
            if soln is not None and len(soln) > 0:
                ...
                errs = []
                DIM_List = [DIM1,DIM2,DIM3,DIM4,DIM5,DIM6,DIM7,SHRINK]
...
def do_slope_recover(LAYER, cluster):
    ...
    print(f"First stage time consume is 2^{math.log2(end_time_1 - start_time_1)}",file=rebuttal_joint_FILE)
    print(f"First stage query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_joint_FILE)
    ...
    if have_null_space == False or len(base_list) < 2:
        ...
        if try_solution.cost > 1e-5 or try_solution.cost < 1e-17 or True:
            ...
            init_soln_num = min(5,len(base_list) - 1)
            for i in range(0,10):
                ...
                print(tmp_slope_list,file=RESULT_FILE)
                print("Index " + str(i) + " residue extraced slope err is",file=RESULT_FILE)
                print(tmp_err,file=RESULT_FILE)
                if part_try_solution.cost < 1e-10:
                    print("good enough",file=RESULT_FILE)
                    break
    end_time_2 = time.perf_counter()
    print(f"Second stage time consume is 2^{math.log2(end_time_2 - start_time_2)}",file=rebuttal_joint_FILE)
    print(f"Second stage query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_joint_FILE)
    ...
    print(f"Third stage time consume is 2^{math.log2(end_time_3 - start_time_3)}",file=rebuttal_joint_FILE)
    print(f"Third stage query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_joint_FILE)
...
if __name__ == '__main__':
    ...
    end_time_1 = time.perf_counter()
    print(f"Extract signature at layer {0} time consume is 2^{math.log2(end_time_1 - start_time_1)}",file=rebuttal_joint_FILE)
    print(f"Extract signature at layer {0} query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_joint_FILE)
    # get slope for layer 1 and signature for layer 2
    start_time_2 = time.perf_counter()
    do_slope_recover(0, pickle.load(open(target_file_name + "/1-cluster-%d.p"%(0 + 1),"rb")))
    end_time_2 = time.perf_counter()
    print(f"Extract slope at layer {0} and signature at layer {1} time consume is 2^{math.log2(end_time_2 - start_time_2)}",file=rebuttal_joint_FILE)
    print(f"Extract slope at layer {0} and signature at layer {1} query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_joint_FILE)
    # get slope for layer 2 and signature for layer 3
    start_time_3 = time.perf_counter()
    do_slope_recover(1, pickle.load(open(target_file_name + "/1-cluster-%d.p"%(1 + 1),"rb")))
    end_time_3 = time.perf_counter()
    print(f"Extract slope at layer {1} and signature at layer {2} time consume is 2^{math.log2(end_time_3 - start_time_3)}",file=rebuttal_joint_FILE)
    print(f"Extract slope at layer {1} and signature at layer {2} query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_joint_FILE)
    # get slope for layer 3 and signature for layer 4
    start_time_4 = time.perf_counter()
    do_slope_recover(2, pickle.load(open(target_file_name + "/1-cluster-%d.p"%(2 + 1),"rb")))
    end_time_4 = time.perf_counter()
    print(f"Extract slope at layer {2} and signature at layer {3} time consume is 2^{math.log2(end_time_4 - start_time_4)}",file=rebuttal_joint_FILE)
    print(f"Extract slope at layer {2} and signature at layer {3} query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_joint_FILE)
    # get slope for layer 4 and signature for layer 5
    # notice that here we using enhanced cluster file for using more dual points
    start_time_5 = time.perf_counter()
    do_slope_recover(3, pickle.load(open(target_file_name + "/1-cluster-enhanced-%d.p"%(3 + 1),"rb")))
    end_time_5 = time.perf_counter()
    print(f"Extract slope at layer {3} and signature at layer {4} time consume is 2^{math.log2(end_time_5 - start_time_5)}",file=rebuttal_joint_FILE)
    print(f"Extract slope at layer {3} and signature at layer {4} query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_joint_FILE)
    start_time_6 = time.perf_counter()
    do_slope_recover(4, pickle.load(open(target_file_name + "/1-cluster-%d.p"%(4 + 1),"rb")))
    end_time_6 = time.perf_counter()
    print(f"Extract slope at layer {4} and signature at layer {5} time consume is 2^{math.log2(end_time_6 - start_time_6)}",file=rebuttal_joint_FILE)
    print(f"Extract slope at layer {4} and signature at layer {5} query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_joint_FILE)
    # get slope for layer 1 and signature for layer 2
    start_time_7 = time.perf_counter()
    do_slope_recover(5, pickle.load(open(target_file_name + "/1-cluster-%d.p"%(5 + 1),"rb")))
    end_time_7 = time.perf_counter()
    print(f"Extract slope at layer {5} and signature at layer {6} time consume is 2^{math.log2(end_time_7 - start_time_7)}",file=rebuttal_joint_FILE)
    print(f"Extract slope at layer {5} and signature at layer {6} query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_joint_FILE)
    # get slope for layer 2 and signature for layer 3
    start_time_8 = time.perf_counter()
    do_slope_recover(6, pickle.load(open(target_file_name + "/1-cluster-%d.p"%(6 + 1),"rb")))

    end_time_8 = time.perf_counter()
    print(f"Extract slope at layer {6} and signature at layer {7} time consume is 2^{math.log2(end_time_8 - start_time_8)}",file=rebuttal_joint_FILE)
    print(f"Extract slope at layer {6} and signature at layer {7} query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_joint_FILE)
