...
from deep_layer_all_layer_recovery import is_consistent, CIFAR10NetPrefix, transfer_weights
import time
import math
import deep_layer_utils


rebuttal_cluster_FILE = open(EXP_ROOT_FOLDER_NAME  + FOLDER_NAME + "/cluster_rebuttal.txt","a")

# If readers want to change the list that cluster program use, please edit the root in this function, and redirect 1-cluster... file to avoid confict

def cheat_cluster(layer):
    BOUNDARY_LIST = [0,DIM1,DIM1 + DIM2,DIM1 + DIM2 + DIM3,
                     DIM1 + DIM2 + DIM3 + DIM4,DIM1 + DIM2 + DIM3 + DIM4 + DIM5,
                     DIM1 + DIM2 + DIM3 + DIM4 + DIM5 + DIM6,DIM1 + DIM2 + DIM3 + DIM4 + DIM5 + DIM6 + DIM7,
                     DIM1 + DIM2 + DIM3 + DIM4 + DIM5 + DIM6 + DIM7 + SHRINK]
    num = 0
    tmp_file = open("process.txt","w")
    duals = []
    root = EXP_ROOT_FOLDER_NAME +FOLDER_NAME + "/enhanced_list/"
    ...
    pickle.dump(cheating, open(EXP_ROOT_FOLDER_NAME +FOLDER_NAME + "/1-cluster-enhanced-%d.p"%layer, "wb"))
...
if len(sys.argv) > 2 and sys.argv[2] == 'slow':
    ...
    print(f"cheat cluster at layer {int(sys.argv[1])} time consume is 2^{math.log2(end_time - start_time)}",file=rebuttal_cluster_FILE)
    print(f"cheat cluster at layer {int(sys.argv[1])} query consume is 2^{deep_layer_utils.query_num}",file=rebuttal_cluster_FILE)
