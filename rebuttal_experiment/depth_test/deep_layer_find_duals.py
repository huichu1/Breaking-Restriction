...
rebuttal_FILE = open(EXP_ROOT_FOLDER_NAME  + FOLDER_NAME + "/find_dual_rebuttal.txt","a")
...
def main():
    ...
    np.random.seed(deep_layer_utils.SEED)
    random.seed(deep_layer_utils.SEED)
    ...
    # If you want to redirect the places where dual points stored, please edit the address below here

    if not os.path.exists(EXP_ROOT_FOLDER_NAME + FOLDER_NAME + "/enhanced_list"):
        os.mkdir(EXP_ROOT_FOLDER_NAME + FOLDER_NAME + "/enhanced_list")
    import pickle
    pickle.dump(all_points, open(EXP_ROOT_FOLDER_NAME + FOLDER_NAME + "/enhanced_list/duals_%08d.p"%(random.randint(0, 1000000)),"wb"))
    
    print(f"Time consume: 2^{math.log2(end_time - start_time)} for seed {SEED}",file=rebuttal_FILE)
    print(f"Query consume: 2^{math.log2(deep_layer_utils.query_num)} for seed {SEED} ",file=rebuttal_FILE)
    print("Finished")
