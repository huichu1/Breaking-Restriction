...
from three_layer_all_layer_recovery import is_consistent, CIFAR10NetPrefix, transfer_weights
import time
import math
...
def cheat_cluster(layer):
    BOUNDARY_LIST = [0,DIM1,DIM1 + DIM2,DIM1 + DIM2 + SHRINK]
    num = 0
    ...
    print("all num is: " + str(num),file=rebuttal_cluster_FILE) 
    pickle.dump(cheating, open(EXP_ROOT_FOLDER_NAME +FOLDER_NAME + "/1-cluster-test-%d.p"%layer, "wb"))
...
if len(sys.argv) > 2 and sys.argv[2] == 'slow':
    ...
    print(f"cheat cluster at layer {int(sys.argv[1])} query consume is 2^{three_layer_utils.query_num}",file=rebuttal_cluster_FILE)
