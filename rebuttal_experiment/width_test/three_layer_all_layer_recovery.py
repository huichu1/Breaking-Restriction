...
import time
import three_layer_utils
import math

rebuttal_joint_FILE = open(EXP_ROOT_FOLDER_NAME  + FOLDER_NAME + "/joint_rebuttal-test.txt","a")
...
def is_consistent_help(points, prefix, layer=0, do_return_soln=False, allow_close=False,step_size = 3000):
    ...
    print("Input points len in " + str(layer) + " is " + str(len(points)),file=rebuttal_joint_FILE)
    ...
    while OK_for_solution == False:
        points = tmp_points
        print("Filtered points len in " + str(layer) + " is " + str(len(points)),file=rebuttal_joint_FILE)
        samples = []
...
def is_consistent_slope_help(points, prefix, layer=0, do_return_soln=False, allow_close=False,step_size = 3000):
    ...
    samples = []
    global tmp_file
    print("Input points len for slope in " + str(layer) + " is " + str(len(points)),file=rebuttal_joint_FILE)
    # The points need to be in different linear regions to try and compare them
    if vectorized_check_closest_pair_distance(points) and not allow_close:
    ...
    while OK_for_solution == False:
        points = tmp_points
        print("Filtered points len for slope in " + str(layer) + " is " + str(len(points)),file=rebuttal_joint_FILE)
        samples = []
        for i, (left, x0, right) in enumerate(points):
            left = np.array(left)
            right = np.array(right)
...
def dosteal(LAYER, cluster):
        ...
        if True:
            ...
            if soln is not None and len(soln) > 0:
                ...
                if min(errs) < 1e-3:
                    ...
                    print("Ok for reconstruction!",file=rebuttal_joint_FILE)
                else:
    ...
    print(f"First stage query consume is 2^{three_layer_utils.query_num}",file=rebuttal_joint_FILE)
...
def do_slope_recover(LAYER, cluster):
            ...
            if soln is not None and len(soln) > 0:
                ...
                if WEIGHTS_DIM_LIST[LAYER + 1] <= WEIGHTS_DIM_LIST[LAYER]:
                    ...
                    if np.min(errs) < 1e-3:
                        ...
                        print("Successfully extracted neuron", np.argmin(errs),
                          'with abs err', np.min(errs),file=RESULT_FILE)
                        print("Ok for non-expansive!",file=rebuttal_joint_FILE)
                else:
                    print("Ok for this?",file=rebuttal_joint_FILE)
    ...
    print(f"Second stage query consume is 2^{three_layer_utils.query_num}",file=rebuttal_joint_FILE)
    ...
    print(f"Third stage query consume is 2^{three_layer_utils.query_num}",file=rebuttal_joint_FILE)
...
if __name__ == '__main__':
    ...
    print(f"Extract signature at layer {0} time consume is 2^{math.log2(end_time_a - start_time_a)}",file=rebuttal_joint_FILE)
    print(f"Extract signature at layer {0} query consume is 2^{three_layer_utils.query_num}",file=rebuttal_joint_FILE)
    ...
    print(f"Extract slope at layer {0} and signature at layer {1} time consume is 2^{math.log2(end_time_b - start_time_b)}",file=rebuttal_joint_FILE)
    print(f"Extract slope at layer {0} and signature at layer {1} query consume is 2^{three_layer_utils.query_num}",file=rebuttal_joint_FILE)
    ...
    print(f"Extract slope at layer {1} and signature at layer {2} time consume is 2^{math.log2(end_time_c - start_time_c)}",file=rebuttal_joint_FILE)
    print(f"Extract slope at layer {1} and signature at layer {2} query consume is 2^{three_layer_utils.query_num}",file=rebuttal_joint_FILE)
